...
(* This is taken from the ocsigenserver project, adjusted to avoid dependency on
 * ocsigen configuration and to use logs instead of logs_lwt.  It is meant as a
 * temporary solution for InhCA until replaced by a customized SQL schema. *)

module Log = (val Logs.src_log (Logs.Src.create "ocsigen:ocsipersist:sqlite"))
...
let db_file = ref Config.(global.ocsidb_file)
...
let close_safely db =
 if not (db_close db) then
   Log.err (fun f -> f "Couldn't close database")
